...
(defun NEXT-STATE (S A)
	(cond
		((equal A 'h)
			(cond
				((and (equal (second S) (third S)) (equal (second S) (first S)))
					NIL)
				((and (equal (second S) (fourth S)) (equal (second S) (first S)))
					NIL)
				(t 
		...
		((equal A 'b)
			(cond
				((not(equal (first S) (second S)) )
					NIL)
				;this action is guaranteed safe
				(t (list (append (list (not (first S))) (list (not (second S))) (list (third S)) (list (fourth S)))))
				)
			)
		((equal A 'd)
			(cond
...
(defun MULT-DFS (STATES PATH)
	(cond 
		((not PATH) NIL)
		;add base cases
		((FINAL-STATE (car PATH))
			(append STATES (list (car PATH)))
		)
		((ON-PATH (car PATH) STATES)
			(MULT-DFS STATES (cdr PATH))
		)
		(t
			(MULT-DFS (append STATES (list (car PATH))) (SUCC-FN (car PATH)))
		)
	)
)
...
(defun DFS (S PATH)
	(cond
		((not PATH) 
			(MULT-DFS (list S) (SUCC-FN S))
		)
		(t (MULT-DFS PATH (SUCC-FN S)))
	)
)
