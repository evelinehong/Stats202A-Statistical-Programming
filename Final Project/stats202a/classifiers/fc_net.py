...
from stats202a.layer_utils import *
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=1*28*28, hidden_dim=100, num_classes=10, reg=0.0):
        ...
        self.params['W1'] = 0.00001 * np.random.randn(input_dim, hidden_dim)
        self.params['W2'] = 0.00001 * np.random.randn(hidden_dim, num_classes)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        hidden, cache_hidden = fc_relu_forward(X, w1, b1)
        scores, cache_out = fc_forward(hidden, w2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        grad_hidden, grad_w2, grad_b2 = fc_backward(grad_loss, cache_out)
        _, grad_w1, grad_b1 = fc_relu_backward(grad_hidden, cache_hidden)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for layer_id in range(1, self.num_layers):
            ...
            current_x, cache = fc_forward(current_x, self.params['W%d' % layer_id],
                                              self.params['b%d' % layer_id])

            # save cache for backward pass
            caches['fc%d' % layer_id] = cache
        ...
        # final affine layer
        scores, cache_out = fc_forward(current_x, self.params['W%d' % self.num_layers],
        ...
        grad_current_x, dw, db = fc_backward(grad_loss, cache_out)
        ...
        for layer_id in range(self.num_layers - 1, 0, -1):
            ...
            grad_current_x, dw, db = fc_backward(grad_current_x, caches['fc%d' % layer_id])
